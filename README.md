...
5. 按 `Run workflow`
6. 等待完成後，在該次 workflow run 底部下載 artifact：`SGRE-debug-apk-v1.0.19`
7. 解壓縮後得到 `sgre-v1.0.19-debug.apk`
8. 傳到 Android 手機安裝
...
- 首頁卡片高度壓縮，卡片底色改為淡灰色。
- 功率顯示改為整數 W，不顯示小數點後位數。
- 沒資料欄位改為空白，不再強制顯示 `--`。
- 右上角 `+` 的導出 / 匯入改為 Android 檔案方式。
- WebView 增加狀態列安全區避讓，改善上方時間 / 電量重疊。
- 版本更新為 `1.0.18`。

## V16 修正

- 修正壓縮包根目錄仍顯示 V14 的問題。
- 版本提高為 `1.0.19`，避免手機仍顯示 `1.0.17` 或安裝舊版。
- GitHub Actions 產出的 APK 檔名改為 `sgre-v1.0.19-debug.apk`，避免誤拿舊的 `app-debug.apk`。
- 功能內容延續 V15：檔案方式導出 / 匯入、首頁卡片壓縮、淡灰卡片底色、功率整數顯示、空資料留白。
